...
int cross_check_int(struct DxfFileI curr_file, struct DxfFileI pos_file, struct PointI offset, struct PointI pos_offset);

static int vec_prod(int x1, int y1, int x2, int y2);
static int cross_check_with_line(struct PointI lp1, struct PointI lp2, struct PointI vp1, struct PointI vp2);
...
int cross_check_int(struct DxfFileI curr_file, struct DxfFileI pos_file, struct PointI offset, struct PointI pos_offset)
{
	struct PolygonI curr_poly, pos_poly;
    int xl1, xr1, yt1, yb1;
	int xl2, xr2, yt2, yb2;
	int i, k;
	...
	for (i = 0; i < curr_poly.n_points - 1; i++) {
		...
		for (k = 0; k < pos_poly.n_points - 1; k++) {
			int res;
			struct PointI pp1, pp2, cross_point;
