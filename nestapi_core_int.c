...
void dxf_file_new_int(struct DxfFileI* dxf_file, char *path, int quant);

static void read_entity(struct DxfFileI *dxf_file);
static void prepare_to_read(char *path, struct DxfFileI *dxf_file);
static void find_control_dots(struct DxfFileI *dxf_file);
static int is_new_figure(char *line, struct DxfFileI *dxf_file, int curr_prim);
static int is_new_figure2(char *line, struct DxfFileI *dxf_file);
...
void dxf_file_new_int(struct DxfFileI* dxf_file, char *path, int quant)
{
	...
	move_to_zero_int(dxf_file);
	create_polygon_jarvis_int(dxf_file);
	gravity_center_in_polygon_int(dxf_file);
}
	...
	for (i = 0; i < dxf_file->n_primitives; i++) {
		...
		for (k = 0; k < dxf_file->str_count[i]; k++) {
			...
			if (strcmp(line, " 10\r\n") == 0) {
				...
				count++;
				strcpy(line, dxf_file->text_primitives[i].lines[k]);
				dxf_file->primitives[i].points[n].x = atoi(line);
			}
			if (strcmp(line, " 20\r\n") == 0) {
				k++;
				count++;
				strcpy(line, dxf_file->text_primitives[i].lines[k]);
				dxf_file->primitives[i].points[n].y = atoi(line);
			}
			if (count == 2) {
				n++;
				count = 0;
